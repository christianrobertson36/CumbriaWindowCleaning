    ...
    <main>
      ...
      <section id="admin" className="section adminPanel">
        ...
        {!token ? <form className="login" onSubmit={doLogin}><input value={login.email} onChange={e => setLogin({ ...login, email: e.target.value })} /><input type="password" placeholder="Password" value={login.password} onChange={e => setLogin({ ...login, password: e.target.value })} /><button className="button">Login</button></form> : <>
          <div className="tabs">{['dashboard','customers','planner','money','leads'].map(t => <button className={tab === t ? 'active' : ''} onClick={() => setTab(t)} key={t}>{t}</button>)}</div>
          ...
          {tab === 'leads' && <LeadList leads={leads} updateLeadStatus={updateLeadStatus} />}
        </>}
      </section>
    </main>
  </>;
...
function LeadList({ leads, updateLeadStatus }) { return <div className="tableList">{leads.map(l => <article className="row" key={l.id}><div><strong>{l.name} · {l.phone}</strong><span>{l.address} {l.postcode}</span><span>{l.message}</span></div><b>{l.status}</b><button onClick={() => updateLeadStatus(l.id, 'Contacted')}>Contacted</button><button onClick={() => updateLeadStatus(l.id, 'Won')}>Won</button></article>)}</div>; }
